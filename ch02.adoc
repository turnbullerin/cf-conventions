...
The language used in attributes whose contents are not standardized should be consistent.
If the language used is not English, it must be identified in the `locale_default` attribute, as per Chapter 10. 
If more than one language is desired, Chapter 10 outlines how to localize attributes.
...
CF datasets that provide localized (i.e. multilingual) versions of variables will follow the standard pattern for such variables in Chapter 10.
...
CF datasets that provide localized (i.e. multilingual) versions of attributes will follow the standard pattern for such attributes in Chapter 10.
